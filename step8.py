...
def show_step_8_content():
    ...
    def load_custom_bins_excel(uploaded_file):
        # 如果传入的是字符串 URL（从 GitHub 读取），先用 requests 下载到内存，
        # 避免 pd.read_excel 直接处理 URL 时触发 ASCII 编码错误
    ...
    def calc_stack_distribution(df_raw, selected_types, latest_year, calc_func, x_bins_custom=None):
        ...
        # ── 2. 确定分箱边界，必须完整覆盖所有实际值 ──
        if x_bins_custom and len(x_bins_custom) > 1:
            bins = sorted(set(x_bins_custom))
    
            # 🌟 关键：若实际数据超出用户指定范围，自动扩展边界
            actual_min = min(ratios_list)
            actual_max = max(ratios_list)
            if actual_min < bins[0]:
                bins = [actual_min] + bins   # 向左扩展，覆盖负值
            if actual_max > bins[-1]:
                bins = bins + [actual_max]   # 向右扩展，覆盖超出值
        else:
            # 自动分箱，天然覆盖全部数据范围
            actual_min = min(ratios_list)
            actual_max = max(ratios_list)
    ...
    def calc_industry_profit_mix(df, company_type, year):
        ...
        for co in companies:
            ...
            if total_profit == 0:
                continue
            ratios = {"保险利润": insurance_profit / total_profit * 100, "投资利润": investment_profit / total_profit * 100}
    ...
    #保险服务收入的构成 - 非PAA合同组的收入构成
    def calc_industry_composition(df, company_type, year, numerator_fields, denominator_field=None):
        """计算单个公司类型的行业平均构成
        与 Step 7 一致：分母 = 各分子项的绝对值之和
        分类平均：先计算每家公司占比，再按公司类型求平均
        """
        mask = (df['公司类型'] == company_type) & (df['报告年份'].astype(str) == str(year))
        df_filtered = df[mask].copy()
        if df_filtered.empty:
        ...
        for co in companies:
            ...
            for field in numerator_fields:
                val = df_co[df_co['字段名'] == field]['(百万)人民币'].sum()
                vals[field] = val if pd.notna(val) else 0
            total = sum(abs(vals[f]) for f in numerator_fields)
            if total == 0:
                continue
            ratios = {}
            for field in numerator_fields:
                ratio = vals[field] / total * 100
                ratios[field] = ratio
            company_ratios.append(ratios)
        if not company_ratios:
            return None
        ratios_df = pd.DataFrame(company_ratios)
        return {field: ratios_df[field].mean() for field in numerator_fields}
    
    #保险服务收入的构成 - PAA和非PAA合同组
    def calc_industry_two_cat_composition(df, company_type, year, fields):
        ...
        for co in companies:
            ...
            if total == 0:
                continue
            ratios = {f: df_co[df_co['字段名'] == f]['(百万)人民币'].sum() / total * 100 for f in fields}
            company_ratios.append(ratios)
        if not company_ratios:
            return None
        ratios_df = pd.DataFrame(company_ratios)
    ...
    def calc_industry_expense_composition(df, company_type, year):
        ...
        for co in companies:
            df_co = df_filtered[df_filtered['公司'] == co]
            vals = {f: df_co[df_co['字段名'] == f]['(百万)人民币'].sum() for f in EXPENSE_FIELDS}
            total = sum(abs(vals[f]) for f in EXPENSE_FIELDS)
            if total == 0:
                continue
            for f in EXPENSE_FIELDS:
                all_ratios[f].append(vals[f] / total * 100)
        if not all_ratios[EXPENSE_FIELDS[0]]:
            return None
        return {f: np.mean(all_ratios[f]) for f in EXPENSE_FIELDS}
    ...
    def calc_industry_expense_structure(df, company_type, year):
        ...
        for co in companies:
            ...
            if total == 0:
                continue
            ratios = {f: vals[f] / total * 100 for f in expense_fields}
            company_ratios.append(ratios)
            company_totals.append(total)
        if not company_ratios:
            return None
        ratios_df = pd.DataFrame(company_ratios)
        return {'ratios': {f: ratios_df[f].mean() for f in expense_fields}, 'avg_total': np.mean(company_totals)}  
    ...
    def calc_industry_profit_composition(df, company_type, year):
        ...
        if df_filtered.empty:
            return None, []
        companies = df_filtered['公司'].unique()
        company_contributions, excluded_companies = [], []
        for co in companies:
            df_co = df_filtered[df_filtered['公司'] == co]
    ...
    def calc_industry_csm_composition(df, company_type, year):
        ...
        for co in companies:
            df_co = df_filtered[df_filtered['公司'] == co]
            vals = {f: df_co[df_co['字段名'] == f]['(百万)人民币'].sum() for f in CSM_FIELDS}
            total = sum(abs(vals[f]) for f in CSM_FIELDS)
            if total == 0:
                continue
            for f in CSM_FIELDS:
                all_ratios[f].append(vals[f] / total * 100)
        if not all_ratios["新业务CSM（集团口径）"]:
            return None
        return {f: np.mean(all_ratios[f]) for f in CSM_FIELDS}

    #————————————堆叠分布图————————————    
    #CSM/BEL 占比
    def calc_csm_bel_ratio(df_co):
        """CSM/BEL占比 = CSM期末余额 / (LRC非亏损部分期末 + LRC亏损部分期末 - CSM期末余额) × 100%"""
        csm = df_co[df_co['字段名'].str.contains('CSM期末', na=False)]['(百万)人民币'].sum()
        lrc_non_loss = df_co[df_co['字段名'].str.contains('LRC非亏损部分期末', na=False)]['(百万)人民币'].sum()
        lrc_loss = df_co[df_co['字段名'].str.contains('LRC亏损部分期末', na=False)]['(百万)人民币'].sum() 
        denom = lrc_non_loss + lrc_loss - csm
        return csm / denom * 100 if denom != 0 else np.nan
    ...
    def calc_nb_ifrs_margin(df_co):
        """新业务IFRS利润率 = (新业务CSM + 新业务LC) / 新业务未来现金流入现值 × 100%"""
        ...
        return (nb_csm + lc) / pv * 100 if pv != 0 else np.nan
    ...
    SCATTER_AXIS_META = {
        ...
        "industry_csm_equity": {"title": "综合净资产", "x_field": "综合净资产", "y_field": "综合净资产增长率", "x_label": "综合净资产区间（亿元）", "y_label": "综合净资产增长率区间（%）", "default_x": "0,500,1000,2000,5000,10000", "default_y": "-50,-20,0,20,50,100"},
        ...
        "industry_nb_lost": {"title": "新业务LC", "x_field": "新业务LC", "y_field": "新业务LC增长率", "x_label": "新业务LC金额（亿元）", "y_label": "新业务LC增长率区间（%）", "default_x": "-100,-50,-20,-10,0,10,20,50", "default_y": "-100,-50,-20,0,20,50,100"},
    }
    ...
    for _m in [
        ...
        "industry_prof_2025", "industry_prof_2024", "industry_prof_mix", "industry_asset_struct",
    ...
    def generate_custom_analysis(m_id, df, cos, cy, py):
            ...
            try:
                ...
                if category == "stack_dist":
                    ...
                    # 🌟 优先读主图渲染时缓存的 labels 和 distribution_df
                    # 保证文字区间和图表 x 轴 100% 一致
                    cached_labels = st.session_state.get(f"stack_dist_labels_{m_id}")
                    cached_df = st.session_state.get(f"stack_dist_df_{m_id}")
                
                    if cached_labels and cached_df is not None and not cached_df.empty:
                        # ✅ 有缓存，直接用
                        labels = cached_labels
                        dist_df = cached_df
                    else:
                         # 🔄 没有缓存时，与图表渲染走完全相同的 bins 决策路径
                         uploaded_x_gen, _ = get_uploaded_bins_by_mid(m_id)
                         if uploaded_x_gen:
                             # 有上传刻度 → 直接用，与打印/渲染保持一致
                             x_bins = uploaded_x_gen
                         else:
                             df_year_tmp = df[df['报告年份'].astype(str) == str(cy)].copy()
                             ratios_tmp = []
                             for co_tmp in df_year_tmp['公司'].unique():
                                 r_tmp = meta["calc_func"](df_year_tmp[df_year_tmp['公司'] == co_tmp])
                                 if r_tmp is not None and not np.isnan(r_tmp):
                                     ratios_tmp.append(r_tmp)
                             if ratios_tmp:
                                 ...
                                 auto_bins_str = meta.get("default_x", "")
                             x_bins, _ = get_axis_bins_for_mid(m_id, auto_bins_str, "")
                         dist_df, labels, _co_names_unused = calc_stack_distribution(
    ...
    def get_company_sort_map(df_raw, latest_year, sort_field):
        ...
        - "综合净资产"：期末股东权益 + CSM期末余额 × 0.75（与 calc_scatter_data 中口径一致）
        - 其它字段：直接按"字段名"列中同名取值（"(百万)人民币"列求和），
          这样未来新增排序字段（如总资产）时，只要原始数据里有该字段名，无需改代码。
        """
        df_year = df_raw[
            df_raw['报告年份'].astype(str).str.replace('.0', '', regex=False) == str(latest_year)
        ]
        sort_map = {}
        if sort_field == "综合净资产":
            for co, g in df_year.groupby('公司'):
                eq = g[g['字段名'] == '期末股东权益']['(百万)人民币'].sum()
                csm = g[g['字段名'] == 'CSM期末余额']['(百万)人民币'].sum()
                sort_map[co] = eq + csm * 0.75
        else:
            for co, g in df_year.groupby('公司'):
    ...
    def create_scatter_chart(df_scatter, config, x_label, y_label):
        ...
        # 🌟 同类型内的公司展示顺序：优先按【行业分析配置】里选定的固定字段排序
        # （例如总资产/综合净资产），而不是按当前这张图自己的 x 轴字段排序。
        # 如果该字段在数据里完全取不到值（比如原始数据还没有"总资产"这一行），
        # 则自动回退为按当前图表的 x_col 排序，保证图表始终能正常显示。
        ...
        # 🌟 用 Shape 矩形替代 go.Heatmap，整张图回到 SVG 渲染，打印颜色与屏幕完全一致
        for i in range(ny):
            for j in range(nx):
        ...
        # 🌟 直接赋值 layout 对象，绕开 update_* 方法可能被 autorange 覆盖的问题
        fig.layout.xaxis.range = [0, nx]
        fig.layout.xaxis.autorange = False
        fig.layout.yaxis.range = [0, ny]
        fig.layout.yaxis.autorange = False

        # 用 Shape 画外框代替 showline，精确贴合热力矩形边界
        fig.add_shape(type="rect", xref="x", yref="y",
                      x0=0, x1=nx, y0=0, y1=ny,
    ...
    def create_stack_chart_and_table(distribution_df, labels, metric_name, target_year, show_labels, label_size, company_names_in_bin=None):
        ...
        company_names_in_bin = company_names_in_bin or {}
        
        fig = go.Figure()
        for ct in ct_list:
            values = distribution_df.loc[ct].values
            color = COMPANY_TYPE_COLORS.get(ct, "#94A3B8")
            # 🌟 悬浮提示：类别（颜色/图例）和区间（x 轴）已经能看出来了，
            # 这里只展示「具体是哪几家公司」这条增量信息，按 company_sort_map 的口径排序展示
            names_per_bin = company_names_in_bin.get(ct, {})
            hover_texts = []
            for lbl in labels:
                names = names_per_bin.get(lbl, [])
                names_sorted = sorted(names, key=lambda c: company_sort_map.get(c, float('-inf')), reverse=True)
                hover_texts.append("、".join(names_sorted) if names_sorted else "（无公司）")
    ...
    def create_composition_chart_v2(results_by_year, year_list, selected_types, config):
        ...
        fig.update_layout(
            ...
            bargap=0.2,
            height=380,
            margin=dict(t=50, b=10, l=150, r=150),
            legend=dict(
                orientation="h",
                yanchor="top",
                y=-0.08,
    ...
    def create_expense_chart(results_by_year, year_list, selected_types, config):
        ...
        fig.update_layout(barmode="stack", bargap=0.05, height=380, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                          margin=dict(t=100, b=10, l=150, r=150),
    ...
    def create_profit_mix_chart(results_by_year, year_list, selected_types, config):
        ...
        fig.update_layout(barmode="group", bargap=config["gap"], bargroupgap=0, height=380,
                          margin=dict(t=40, b=65, l=200, r=200),
                          paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
    ...
    def create_industry_asset_composition_chart(results_by_year, year_list, selected_types, config):
        ...
        fig.update_layout(barmode='stack', height=380, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                          margin=dict(t=50, b=120, l=150, r=150),
                          legend=dict(orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5, font=dict(size=11)))
    ...
    def render_pure_chart_entity(m_id, print_mode):
        ...
        if m_id in SCATTER_AXIS_META:
            ...
            if df_scatter.empty:
                ...
                return
            # 🌟 calc_scatter_data 返回的列名是 display_name（即 meta["title"]）
            # 和 f"{title}增长率"，而不是 meta["x_field"]/meta["y_field"]
            # （两者在多数指标下恰好同名，但如 industry_nb_lost 等指标
            #  title 与 x_field 不同，需要用 title 取列才不会 KeyError）
        ...
        # 统一处理所有堆叠分布图
            ...
            use_upload_key = f"use_uploaded_bins_{m_id}"   # ← 不变
            x_text_key     = f"x_bins_text_{m_id}"         # ← 不变
            ...
            if not print_mode:
                ...
                with btn_col:
                    with st.popover(f"⚙️ {meta['name']} 参数设置", use_container_width=True):
                        ...
                        if uploaded_x:
                            ...
                            st.session_state[use_upload_key] = True   # 强制始终用上传值
                        else:
                ...
                if ratios_tmp:
                    ...
                    auto_bins_str = meta["default_x"]  # 数据为空时兜底
            
                x_bins, _ = get_axis_bins_for_mid(m_id, auto_bins_str, "")
            
            else:
                # 打印模式：上传Excel刻度优先，否则用自动计算
                uploaded_x_print, _ = get_uploaded_bins_by_mid(m_id)
                if uploaded_x_print:
                    # 有上传刻度 → 直接用，跳过自动计算
                    st.session_state[f"use_uploaded_bins_{m_id}"] = True
                    st.session_state[f"x_bins_text_{m_id}"] = bins_to_str(uploaded_x_print)
            ...
            # 🌟 把这次图表用的 labels 和 distribution_df 存入 session_state
            # 供 generate_custom_analysis 直接复用，保证文字和图完全一致
            ...
            create_stack_chart_and_table(
                ...
                company_names_in_bin=company_names_in_bin
            )
        ...
        #IFRS9 资产端分类        
            ...
            if not print_mode:
                ...
                with c2:
                    bar_width = st.slider("柱子宽度", 0.1, 1.0, 0.35, key=f"wid_{m_id}")
                with c3:
                    label_size = st.slider("标签大小", 8, 16, 11, key=f"sz_{m_id}")
            else:
